...
func InitBlockChain() *BlockChain {
  ...
  err = db.Update(func(txn *badger.Txn) error {
    // check if blocks exist, if not create the genesis block
    if _, err := txn.Get([]byte("lh")); err == badger.ErrKeyNotFound { // "lh" = last hash key
      fmt.Println("No existing blockchain found")
      genesis := Genesis() // create the genesis block since blockchain does not exist
      ...
      return err
    } else { // blockchain already exists
